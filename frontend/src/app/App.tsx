...
function ValueSection() {
  const values = [
    { num: "01", title: "Прямые фабрики", desc: "Работаем напрямую с производителями — без посредников, без наценок. Ваша цена = фабричная цена." },
    { num: "02", title: "Лучшая цена", desc: "Цены на 30–60% ниже рыночных. Оптовые условия от 50 единиц. Гибкая ценовая политика." },
    { num: "03", title: "Контроль качества", desc: "Выездной контроль на производстве. Проверка партии перед отгрузкой. Сертификация по запросу." },
    { num: "04", title: "Custom Production", desc: "Ваш лейбл, ваш цвет, ваш состав. Производство под бренд от 100 единиц. Полный white-label." },
  ];
  return (
      ...
      <div className="max-w-7xl mx-auto px-8 md:px-16">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-20 gap-6">
          <div>
            ...
            <h2 style={{ fontSize: "clamp(2rem, 5vw, 3.5rem)", fontWeight: 800, letterSpacing: "-0.02em", color: "#0a0a0a", lineHeight: 1.1 }}>Почему работают<br />с нами</h2>
          </div>
          <p style={{ fontSize: "0.85rem", fontWeight: 400, color: "#888", maxWidth: "280px", lineHeight: 1.7, letterSpacing: "0.01em" }}>Мы — прямой мост между вашим бизнесом и лучшими трикотажными фабриками Китая.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
          {values.map((v, i) => (
            <div key={v.num} style={{ borderTop: `2px solid ${i === 0 ? RED : "#ddd"}`, paddingTop: "2rem", paddingRight: "2.5rem", paddingBottom: "2rem" }}>
              <span style={{ display: "block", fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.2em", color: RED, marginBottom: "1.2rem" }}>{v.num}</span>
              <h3 style={{ fontSize: "1.05rem", fontWeight: 700, letterSpacing: "-0.01em", color: "#0a0a0a", marginBottom: "0.8rem", lineHeight: 1.3 }}>{v.title}</h3>
              <p style={{ fontSize: "0.8rem", fontWeight: 400, color: "#666", lineHeight: 1.75 }}>{v.desc}</p>
            </div>
          ))}
        </div>
        <div className="mt-24 border-t border-black/10 pt-12 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[{ stat: "200+", label: "Фабрик-партнёров" }, { stat: "50+", label: "Стран поставок" }, { stat: "5 лет", label: "На рынке" }, { stat: "10K+", label: "Клиентов B2B" }].map((s) => (
            <div key={s.stat}>
              <p style={{ fontSize: "2.5rem", fontWeight: 800, color: "#0a0a0a", letterSpacing: "-0.03em", lineHeight: 1, marginBottom: "0.5rem" }}>{s.stat}</p>
              <p style={{ fontSize: "0.72rem", fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", color: "#999" }}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="category" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: W }} className="py-28 md:py-36">
      <div className="max-w-7xl mx-auto px-8 md:px-16">
        <div className="relative mb-0">
          ...
          <p style={{ position: "absolute", bottom: "1.5rem", left: "0.3rem", fontSize: "0.7rem", fontWeight: 600, letterSpacing: "0.25em", textTransform: "uppercase", color: RED }}>Коллекция 2025</p>
        </div>
